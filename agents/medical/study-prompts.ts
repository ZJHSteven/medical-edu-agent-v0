...
请基于本会话中已经发生的问诊、第一次判断、临床反馈、Europe PMC 循证过程和最终反思，输出一份简洁但结构化的 Markdown 总结，固定包含：
1. “本次推理轨迹”：概括初判到终判最重要的变化；
2. “Evidence Ledger（证据账本）”：只列本轮真实检索/阅读过且确实进入判断的证据，说明其支持、反驳或未改变什么判断；没有足够证据时明确写不足，不得编造文献；
3. “认知偏差复盘”：结合学生自己的反思讨论可能存在或被避免的锚定、过早闭合、确认偏误等，不确定时明确写“未能确定”；
4. “形成性评价”：按信息采集、鉴别诊断、证据整合、反思修订四个维度各给 1~5 分并各用一句话解释；这只是 AI 形成性评价，不得表述为正式研究评分或教师评分；
5. “下一步建议”：给出 2~3 条具体学习建议。
Evidence Ledger 和形成性评价优先使用 Markdown 表格，方便完成页直接结构化展示。
