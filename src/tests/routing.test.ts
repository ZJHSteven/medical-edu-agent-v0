...
async function familySessionCookie(): Promise<string> {
  const response = await assistantWorker.fetch(
    new Request("http://example.com/auth/login", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ username: "A", password: "AAA" })
    }),
    env,
...
describe("AssistantDirectory — onBeforeSubAgent strict-registry gate", () => {
  ...
  it("protects the directory URL, then routes it for an authenticated family session", async () => {
    const anonymous = await assistantWorker.fetch(
    ...
    const cookie = await familySessionCookie();
  ...
  it("routes the friendly chat URL through the authenticated Worker entry", async () => {
    const directory = await getAgentByName(env.AssistantDirectory, "A");
